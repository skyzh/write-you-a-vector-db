...
fn main() -> vector_core::Result<()> {
    ...
    let dataset = workload.dataset.clone();
    let started = Instant::now();
    let nsw = NswIndex::try_new(dataset, workload.metric, nsw_config())?;
    let nsw_build = started.elapsed();

    let dataset = workload.dataset.clone();
    let started = Instant::now();
    let hnsw = HnswIndex::try_new(dataset, workload.metric, hnsw_config())?;
    let hnsw_build = started.elapsed();

    let dataset = workload.dataset.clone();
    let started = Instant::now();
    let ivf_pq = IvfPqIndex::try_new(dataset, workload.metric, ivf_pq_config())?;
