...
fn vector_scan_rows(batches: &[RecordBatch]) -> Result<Vec<Vec<String>>, DataFusionError> {
    ...
    let scans = physical_plan
        .lines()
        .map(str::trim)
        .filter(|line| {
            line.starts_with("VectorIndexScanExec") || line.starts_with("VectorScanExec")
        })
        .map(|line| vec![line.to_owned()])
        .collect::<Vec<_>>();
    if scans.is_empty() {
        return Err(DataFusionError::Execution(
            "EXPLAIN omitted the vector scan".into(),
        ));
    }
    Ok(scans)
}
...
fn rows() -> Vec<VectorRow> {
    (0..8)
        .map(|id| VectorRow::new(id, vec![id as f32, 1.0, 1.0], format!("point-{id}")))
        .collect()
}
