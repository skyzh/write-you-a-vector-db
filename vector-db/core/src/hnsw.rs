        ...
        for row in 0..dataset.len() {
            ...
            for current_level in (0..=level.min(top_level)).rev() {
                ...
                for owner in affected {
                    let previous = layers[current_level][owner].clone();
                    prune_neighbors(
                        &dataset,
                        metric,
                        owner,
                        &mut layers[current_level][owner],
                        config.max_connections,
                    );
                    for removed in previous {
                        if !layers[current_level][owner].contains(&removed) {
                            layers[current_level][removed].retain(|neighbor| *neighbor != owner);
                        }
                    }
                }
