...
level must be identical. Non-deterministic levels make the graph
structure unreproducible across runs.

Your budget must satisfy `max_connections > 0`,
`ef_construction >= max_connections`, `ef_search > 0`, and
`max_level > 0`.
...
layer zero produces results inconsistent with the upper-layer
traversal.
