...
The reused C++ diagrams below separate the two frontiers. Candidate queue `C`
chooses the next node to explore; bounded result heap `W` tracks the best rows
seen so far.

![Two entry points initialize the candidate queue and result heap](./vector-db/05-nsw-explore-3.svg)

![Expanding the nearest candidate adds its neighbors to both frontiers](./vector-db/05-nsw-explore-4.svg)

![Search stops when the best pending candidate is worse than the result frontier](./vector-db/05-nsw-explore-8.svg)
...
Insertion uses the same search, then prunes any node whose degree exceeds the
configured bound.

![Insertion first finds the new row's nearest graph neighbors](./vector-db/05-nsw-insert-1.svg)

![Pruning retains only the configured nearest connections](./vector-db/05-nsw-insert-4.svg)
