...
probing more partitions than exist silently wastes work on duplicate
scans.
...
use the same distance metric. Mixing metrics — assigning with Euclidean
but scoring with cosine — produces a result set ordered by a criterion
the probe loop never optimized.
