...
Before the vector-index rewrite, the first query has this plan shape:
...
The required positive cases have one of these two leaves below `Sort`:

- `SeqScan`; or
- `Projection` directly over `SeqScan`.

The projection may reorder columns or compute an extra distance expression. Trace the sort expression's
`ColumnValueExpression` through the projection to find the original table column. Do not assume that the vector column is
always the first projected column.
...
Here the sort expression is a column reference to the projection's computed distance. Trace it one additional step before
applying the same safety contract.
